...
import { Box, Card, CardContent, CardHeader, makeStyles, Typography, useTheme } from '@material-ui/core';
...
const LatestAlert = () => {
  ...
  const { latestSurvey: { yes = 0, no = 0, sent = 0 } } = useSelector(state => state.notifications);
  ...
  const chartColors = useMemo(() => [theme.palette.info.main, theme.palette.score.main, theme.palette.survey.main], [theme]);
  ...
  return (
    <Card className={classes.root}>
      <CardHeader title={selectedGame ? 'Latest Quick Poll' : 'Latest Survey'} />
      <CardContent>
        <div className={classes.container}>
          <div className={classes.chart}>
            <div className={classes.chartLabel}>
              <Typography variant='body2' color='textPrimary' className={classes.sent}>
                {sent}
              </Typography>
